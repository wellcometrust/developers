...
if(result.errors.length>1){result.errors.length=1;}const error=result.errors.join('\n\n');console.error(_chalk.default.red('Failed to compile.\n'));console.error(error);console.error();if(error.indexOf('private-next-pages')>-1){throw new Error('> webpack config.resolve.alias was incorrectly overriden. https://err.sh/zeit/next.js/invalid-resolve-alias');}throw new Error('> Build failed because of webpack errors');}else if(result.warnings.length>0){console.warn(_chalk.default.yellow('Compiled with warnings.\n'));console.warn(result.warnings.join('\n\n'));console.warn();}else{console.log(_chalk.default.green('Compiled successfully.\n'));}const distPath=_path.default.join(dir,config.distDir);const pageKeys=Object.keys(mappedPages);const manifestPath=_path.default.join(distDir,target==='serverless'?_constants.SERVERLESS_DIRECTORY:_constants.SERVER_DIRECTORY,_constants.PAGES_MANIFEST);const{autoExport}=config.experimental;const staticPages=new Set();const pageInfos=new Map();let pagesManifest={};let customAppGetInitialProps;if(autoExport){pagesManifest=JSON.parse((await fsReadFile(manifestPath,'utf8')));}process.env.NEXT_PHASE=_constants.PHASE_PRODUCTION_BUILD;for(const page of pageKeys){const chunks=(0,_chunkGraphPlugin.getPageChunks)(page);const actualPage=page==='/'?'/index':page;const size=await(0,_utils.getPageSizeInKb)(actualPage,distPath,buildId);const bundleRelative=_path.default.join(target==='serverless'?'pages':`static/${buildId}/pages`,actualPage+'.js');const serverBundle=_path.default.join(distPath,target==='serverless'?_constants.SERVERLESS_DIRECTORY:_constants.SERVER_DIRECTORY,bundleRelative);let isStatic=false;if(autoExport){pagesManifest[page]=bundleRelative.replace(/\\/g,'/');const runtimeEnvConfig={publicRuntimeConfig:config.publicRuntimeConfig,serverRuntimeConfig:config.serverRuntimeConfig};const nonReservedPage=!page.match(/^\/(_app|_error|_document|api)/);if(nonReservedPage&&customAppGetInitialProps===undefined){customAppGetInitialProps=(0,_utils.hasCustomAppGetInitialProps)(target==='serverless'?serverBundle:_path.default.join(distPath,_constants.SERVER_DIRECTORY,`/static/${buildId}/pages/_app.js`),runtimeEnvConfig);if(customAppGetInitialProps){console.warn('Opting out of automatic exporting due to custom `getInitialProps` in `pages/_app`\n');}}if(customAppGetInitialProps===false&&nonReservedPage){if((0,_utils.isPageStatic)(serverBundle,runtimeEnvConfig)){staticPages.add(page);isStatic=true;}}}pageInfos.set(page,{size,chunks,serverBundle,static:isStatic});}if(Array.isArray(configs[0].plugins)){configs[0].plugins.some(plugin=>{if(!plugin.ampPages){return false;}plugin.ampPages.forEach(pg=>{pageInfos.get(pg).isAmp=true;});return true;});}await(0,_writeBuildId.writeBuildId)(distDir,buildId,selectivePageBuilding);if(autoExport&&staticPages.size>0){const exportApp=require('../export').default;const exportOptions={silent:true,buildExport:true,pages:Array.from(staticPages),outdir:_path.default.join(distDir,'export')};const exportConfig={...config,exportPathMap:defaultMap=>defaultMap,experimental:{...config.experimental,exportTrailingSlash:false}};await exportApp(dir,exportOptions,exportConfig);const toMove=await(0,_recursiveReaddir.recursiveReadDir)(exportOptions.outdir,/.*\.html$/);let serverDir='';// remove server bundles that were exported
